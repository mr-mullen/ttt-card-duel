...
const Cards = {
  ...
  init() {
    this.state = {
      decks: [
        { name: 'P1', cards: [], discard: [], drawPile: [] },
        { name: 'P2', cards: [], discard: [], drawPile: [] }
      ],
      hands: [[], []],  // Empty arrays instead of null
      drawCount: 0
    };
  ...
  drawStartingHands() {
    this.drawCards(0, 5);  // Draw more initially
    this.drawCards(1, 5);

    this.renderHands();
    this.renderDrawPiles();
    Game.log('Starting hands drawn! Each player gets 5 cards initially.');
  },

  // Draw a single card per turn (called after each turn)
  drawCardPerTurn(playerIndex) {
    const deck = this.state.decks[playerIndex];
    const hand = this.state.hands[playerIndex];

    // Only draw if hand has less than 5 cards
    if (hand.length < 5 && deck.cards.length > 0) {
  ...
  drawCards(playerIndex, count) {
    ...
    for (let i = 0; i < count; i++) {
      if (deck.cards.length > 0) {
        ...
        card.position = i;
        hand.push(card);
        this.renderHands();  // Render after each card for animation
        this.renderDrawPiles();
        Game.log(`${Game.state.currentPlayer} drew a card!`);
        ...
        this.renderHands();
        this.renderDrawPiles();
        Game.log(`${Game.state.currentPlayer} drew from discard!`);
      } else {
        // No cards left
        Game.log(`${Game.state.currentPlayer}'s deck is empty!`);
      }
    }
  ...
  // Draw 1 card per turn (called during turn setup)
  ...
  renderHands() {
    ...
    hands.forEach((handEl, playerIndex) => {
      ...
      if (!hand || hand.length === 0) {
        // No cards yet, just show deck and discard areas
        return;
      }

      hand.forEach((card, index) => {
        const cardEl = this.createCardElement(card, playerIndex, card.position, index);
  ...
  renderDrawPiles() {
    const p1DeckEl = document.getElementById('p1-deck');
    const p1DiscardEl = document.getElementById('p1-discard');
    const p2DeckEl = document.getElementById('p2-deck');
    const p2DiscardEl = document.getElementById('p2-discard');
    ...
    // Update counts
    if (p1DeckCount) p1DeckCount.textContent = this.state.decks[0].cards.length;
    if (p1DiscardCount) p1DiscardCount.textContent = this.state.decks[0].discard.length;
    if (p2DeckCount) p2DeckCount.textContent = this.state.decks[1].cards.length;
    if (p2DiscardCount) p2DiscardCount.textContent = this.state.decks[1].discard.length;

    // Render deck piles
    if (p1DeckEl) {
      p1DeckEl.innerHTML = '';
      if (this.state.decks[0].cards.length > 0) {
        const deckPile = document.createElement('div');
        deckPile.className = 'deck-pile';
        deckPile.innerHTML = `<span class="deck-count">${this.state.decks[0].cards.length}</span><span class="pile-icon">🃏</span>`;
        p1DeckEl.appendChild(deckPile);
      }
    }

    if (p1DiscardEl) {
      p1DiscardEl.innerHTML = '';
      if (this.state.decks[0].discard.length > 0) {
        const discardPile = document.createElement('div');
        discardPile.className = 'discard-pile';
        discardPile.innerHTML = `<span class="discard-count">${this.state.decks[0].discard.length}</span><span class="pile-icon">🗑️</span>`;
        p1DiscardEl.appendChild(discardPile);
      } else {
        const emptyMsg = document.createElement('div');
        emptyMsg.textContent = 'Empty';
        emptyMsg.style.cssText = 'color:var(--color-dimmed);font-size:10px;';
        p1DiscardEl.appendChild(emptyMsg);
      }
    }

    if (p2DeckEl) {
      p2DeckEl.innerHTML = '';
      if (this.state.decks[1].cards.length > 0) {
        const deckPile = document.createElement('div');
        deckPile.className = 'deck-pile';
        deckPile.innerHTML = `<span class="deck-count">${this.state.decks[1].cards.length}</span><span class="pile-icon">🃏</span>`;
        p2DeckEl.appendChild(deckPile);
      }
    }

    if (p2DiscardEl) {
      p2DiscardEl.innerHTML = '';
      if (this.state.decks[1].discard.length > 0) {
        const discardPile = document.createElement('div');
        discardPile.className = 'discard-pile';
        discardPile.innerHTML = `<span class="discard-count">${this.state.decks[1].discard.length}</span><span class="pile-icon">🗑️</span>`;
        p2DiscardEl.appendChild(discardPile);
      } else {
        const emptyMsg = document.createElement('div');
        emptyMsg.textContent = 'Empty';
        emptyMsg.style.cssText = 'color:var(--color-dimmed);font-size:10px;';
        p2DiscardEl.appendChild(emptyMsg);
      }
    }
  },

  // Create card DOM element
  createCardElement(card, playerIndex, position, index) {
    ...
    cardEl.dataset.index = index;  // Important for click handler
  ...
  handleCardClick(card, playerIndex) {
    if (Game.state.phase !== 'PLAYING' || !Game.state.isPlayerTurn) {
      Game.log('Can only play on your turn!');
      return;
    }
  ...
  drawCard(playerIndex) {
    ...
    // Only draw if hand has less than 6 cards
    if (hand.length < 6 && deck.cards.length > 0) {
      const card = deck.cards.pop();
      card.position = hand.length;
      hand.push(card);
      this.state.drawCount++;
